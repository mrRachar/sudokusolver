...
class SudokuSolver:
                    ...
                    for box in array:
                        if box.is_filled:
                            ...
                            for possible_value in set(box.possible_values):
                                ...
                                if possible_value in BoxGrid.box_values(array):
                                    box.possible_values.remove(possible_value)


    def update_possible_values(self, coordinates: Coordinate):
        if None:
            self.update_all_possible_values()
        x, y = coordinates
        for array in self.grid.get_containing_arrays(coordinates):
            for box in array:
                if box.is_filled:
                    continue
                elif len(box.possible_values) == 1:
    ...
    def update_all_possible_values(self):
